 ...
 ******************************************************/
wiced_result_t wiced_set_timer(unsigned long ms, event_handler_t timer_func, void* arg);
 ...
 */
    ...
    xTaskCreate( application_thread_main, (signed char*)"app_thread", app_stack_size/sizeof( portSTACK_TYPE ), NULL, WICED_PRIORITY_TO_NATIVE_PRIORITY(WICED_APPLICATION_PRIORITY), &app_thread_handle);
#else
    /* Create an initial thread */
...
/* MXCHIP API list */
/* RTOS APS */
int mico_rtos_create_thread( wiced_thread_t* thread, uint8_t priority, const char* name, wiced_thread_function_t function, uint32_t stack_size, void* arg )
{
    int temp = WICED_PRIORITY_TO_NATIVE_PRIORITY(priority);
...
int mico_rtos_delete_thread(wiced_thread_t* thread)
{
    if (thread == NULL) {
        vTaskDelete(NULL);
    } else if( xTaskIsTaskFinished( *thread ) != pdTRUE )
...
int mico_rtos_thread_join(wiced_thread_t* thread)
{
    while ( xTaskIsTaskFinished( *thread ) != pdTRUE )
    {
        host_rtos_delay_milliseconds(10);
...
int mico_rtos_thread_force_awake( wiced_thread_t* thread )
{
    vTaskForceAwake(*thread);
    return 0;
}

int mico_rtos_is_current_thread( wiced_thread_t* thread )
{
    if ( xTaskGetCurrentThread( ) == *thread )
    {
        return 1;
...
void mico_rtos_suspend_thread(wiced_thread_t* thread)
{
    vTaskSuspend(thread);
}

int mico_rtos_init_semaphore( wiced_semaphore_t* semaphore, int count )
{
...
int mico_rtos_get_semaphore( wiced_semaphore_t* semaphore, uint32_t timeout_ms )
{
    if ( pdTRUE == xSemaphoreTake( *semaphore, (portTickType) ( timeout_ms / ms_to_tick_ratio ) ) )
    {
        return 0;
...
int mico_rtos_set_semaphore( wiced_semaphore_t* semaphore )
{
    signed portBASE_TYPE result;
...
int mico_rtos_deinit_semaphore( wiced_semaphore_t* semaphore )
{
    if (semaphore != NULL)
    {
        vQueueDelete( *semaphore );
...
int mico_rtos_init_mutex( wiced_mutex_t* mutex )
{
    /* Mutex uses priority inheritance */
    *mutex = xSemaphoreCreateMutex( );
    if ( *mutex == NULL )
...
int mico_rtos_lock_mutex( wiced_mutex_t* mutex )
{
    if ( xSemaphoreTake( *mutex, WICED_WAIT_FOREVER ) != pdPASS )
    {
        return -1;
...
int mico_rtos_unlock_mutex( wiced_mutex_t* mutex )
{
    if ( xSemaphoreGive( *mutex ) != pdPASS )
    {
        return -1;
...
int mico_rtos_deinit_mutex( wiced_mutex_t* mutex )
{
    vSemaphoreDelete( *mutex );
    return 0;
}
...
int mico_rtos_init_queue( wiced_queue_t* queue, const char* name, uint32_t message_size, uint32_t number_of_messages )
{
    UNUSED_PARAMETER(name);
...
int mico_rtos_push_to_queue( wiced_queue_t* queue, void* message, uint32_t timeout_ms )
{
    signed portBASE_TYPE result;
...
int mico_rtos_pop_from_queue( wiced_queue_t* queue, void* message, uint32_t timeout_ms )
{
    if ( xQueueReceive( *queue, message, ( timeout_ms / ms_to_tick_ratio ) ) != pdPASS )
    {
        return -1;
...
int mico_rtos_deinit_queue( wiced_queue_t* queue )
{
    vQueueDelete( *queue );
    return 0;
}

int mico_rtos_is_queue_empty( wiced_queue_t* queue )
{
    signed portBASE_TYPE result;
...
int mico_rtos_is_queue_full( wiced_queue_t* queue )
{
    signed portBASE_TYPE result;
...
/*Timer */
wiced_result_t mico_init_timer( wiced_timer_t* timer, uint32_t time_ms, timer_handler_t function, void* arg )
{
    wiced_assert("Bad args", timer != NULL);
...
wiced_result_t mico_start_timer( wiced_timer_t* timer )
{
    if ( xTimerStart( timer->handle, WICED_WAIT_FOREVER ) != pdPASS )
    {
        return WICED_ERROR;
...
wiced_result_t mico_stop_timer( wiced_timer_t* timer )
{
    if ( xTimerStop( timer->handle, WICED_WAIT_FOREVER ) != pdPASS )
    {
        return WICED_ERROR;
...
wiced_result_t mico_reload_timer( wiced_timer_t* timer )
{
    if ( xTimerReset( timer->handle, WICED_WAIT_FOREVER ) != pdPASS )
    {
        return WICED_ERROR;
...
wiced_result_t mico_deinit_timer( wiced_timer_t* timer )
{
    if ( xTimerDelete( timer->handle, WICED_WAIT_FOREVER ) != pdPASS )
    {
        return WICED_ERROR;
...
wiced_result_t mico_is_timer_running( wiced_timer_t* timer )
{
    return ( xTimerIsTimerActive( timer->handle ) != 0 ) ? 1 : 0;
}
