...
#include <sys/wait.h>
#endif /* WIN32  elif defined(linux) */
#include <stdio.h>
#include <stdlib.h>
...
unsigned long *bytes_to_read)
{
	...
	while (total_numread != *bytes_to_read) {
		...
		if (total_numread != *bytes_to_read) {
			...
			if ((rem_ptr = remote_CDC_rx_hdr(wl, 0)) == NULL) {
				DPRINT_ERR(ERR, "serial_fragmented_response_fe: Reading CDC header failed");
			}
		}
		free(local_buf);
	}
...
rwl_setinformation_fe(void *wl, int cmd, void* buf,
	...
	switch (remote_type) {
		...
		case REMOTE_SERIAL:
			...
			if ((rem_ptr = remote_CDC_rx_hdr(wl, debug)) == NULL) {
				DPRINT_ERR(ERR, "setinformation_fe: Reading CDC header failed\n");
				return FAIL;
			}
