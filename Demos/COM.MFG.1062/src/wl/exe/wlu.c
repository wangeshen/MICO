...
static int wl_parse_rateset_args(void *wl, wl_rateset_args_t* rs, char **argv);
...
#define MAX_COUNTERS_TEXT_SIZE  (4096)

/* dword align allocation */
static union {
	char bufdata[MAX_COUNTERS_TEXT_SIZE];
	uint32 alignme;
} bufstruct_wlu;
...
var_cmd_t wl_varcmd = {"var", wl_varint, -1, -1, "unrecognized name, type -h for help"};
const char *wlu_av0;
...
/* Semicolon to avoid Eclipse warning about no return (Eclipse Bug 347390) */ ;
	return (ret);
}
...
{
	wl_rateset_args_t rs_args, defrs_args;
	wl_rateset_t defrs;
	int error;
	uint i;
	...
	if (*argv == NULL)
	{
		...
		/* get current rateset */
		if ((error = wlu_iovar_get(wl, "cur_rateset", &rs_args, sizeof(rs_args))) == 0)
		{
	        dump_rateset(rs_args.rates, dtoh32(rs_args.count));
	        printf("\n");
            wl_print_mcsset((char *)rs_args.mcs);
		}
		else if ( error == BCME_UNSUPPORTED )
        {
            memset( &rs_args, 0, sizeof(rs_args) );
            if ( ( error = wlu_get(wl, WLC_GET_RATESET, &rs_args, sizeof(rs_args)) ) < 0 )
                return (error);
            dump_rateset(rs_args.rates, dtoh32(rs_args.count));
            printf("\n");
        }
	...
	else
	{
		...
		if ((error = wlu_iovar_get(wl, "rateset", &defrs_args, sizeof(wl_rateset_args_t))) == 0)
		{
		    defrs_args.count = dtoh32(defrs_args.count);

	        if (!stricmp(*argv, "all"))
	        {
	            for (i = 0; i < defrs_args.count; i++)
	                defrs.rates[i] |= 0x80;
	            defrs_args.count = htod32(defrs_args.count);
	            error = wlu_iovar_set(wl, "rateset", &defrs_args,
	        ...
	        {  /* arbitrary list */
	            wl_parse_rateset_args(wl, &defrs_args, argv);
	            /* check for common error of no basic rates */
	            for (i = 0; i < defrs_args.count; i++) {
	                if (defrs_args.rates[i] & 0x80)
	                    break;
	            }
	            if (i < defrs_args.count) {
	                defrs_args.count = htod32(defrs_args.count);
	                error = wlu_iovar_set(wl, "rateset", &defrs_args,
	                    sizeof(wl_rateset_args_t));
	            } else {
...
wl_sup_rateset(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		if (!stricmp(*argv, "-1") || !stricmp(*argv, "0")) {
			...
			wl_parse_rateset_args(wl, &rs, argv);
...
static int
wl_parse_rateset_args(void *wl, wl_rateset_args_t* rs, char **argv)
{
	char* endp = NULL;
	char* arg;
	int r;
	...
	void *param, int paramlen, void *bufptr, int buflen)
{
	...
	if (err)
	{
	    printf("Error getting buffer for IOVAR\n");
		return err;
	}
...
wl_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	statsbuf = (char *)ptr;
	memcpy(&cnt_hdr, statsbuf, sizeof(cnt_hdr));
