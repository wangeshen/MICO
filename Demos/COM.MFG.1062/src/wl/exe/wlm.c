...
{
	wl_cnt_header_t *cnt_header;

	if (wlu_var_getbuf(irh, "counters", NULL, 0, (void **)&cnt_header)) {
		printf("wlmTxGetAckedPackets: %s\n", wlmLastError());
		return FALSE;
	}

    cnt_header->version = dtoh16(cnt_header->version);
    cnt_header->length = dtoh16(cnt_header->length);

    if (cnt_header->version == 5)
    {
        *count = dtoh32( ((wl_cnt_v5_t*)cnt_header)->rxackucast );
    }
    else
    {
        *count = dtoh32( ((wl_cnt_v6_t*)cnt_header)->rxackucast );
    }
	return TRUE;
}
...
int wlmRxGetReceivedPackets(unsigned int *count)
{
	...
	if (wlu_var_getbuf_med(irh, "counters", NULL, 0, (void **)&cnt_header)) {
	...
	if (cnt_header->version == 5)
	{
		*count = dtoh32( ((wl_cnt_v5_t*)cnt_header)->pktengrxducast );
	}
	else if (cnt_header->version == 6)
    {
        *count = dtoh32( ((wl_cnt_v6_t*)cnt_header)->pktengrxducast );
    }
	else
	{
		*count = dtoh32( ((wl_cnt_v6_t*)cnt_header)->rxdfrmucastmbss);
	}
	...
	WLM_ENCRYPTION encryption, const char *key)
{
	...
	if (wlu_iovar_setint(irh, "auth", authType)) {
		printf("wlmSecuritySet - error setting auth iovar: %s\n", wlmLastError());
		return FALSE;
	}

	if (wlu_iovar_setint(irh, "wpa_auth", wpa_auth)) {
		printf("wlmSecuritySet - error setting wpa_auth iovar: %s\n", wlmLastError());
		return FALSE;
	}

	if (wlu_iovar_setint(irh, "sup_wpa", sup_wpa)) {
		printf("wlmSecuritySet - error setting sup_wpa iovar: %s\n", wlmLastError());
		return FALSE;
	}
	...
	if (encryption == WLM_ENCRYPT_WEP) {
		...
		for (i = 0; i < 4; i++) {
			...
			if (wlu_set(irh, WLC_SET_KEY, k, sizeof(*k))) {
				printf("wlmSecuritySet - error setting SET_KEY ioctl: %s\n", wlmLastError());
				return FALSE;
			}
		}
		...
		if (wlu_set(irh, WLC_SET_KEY_PRIMARY, &primary_key, sizeof(primary_key)) < 0) {
			printf("wlmSecuritySet - error setting SET_KEY_PRIMARY ioctl: %s\n", wlmLastError());
			return FALSE;
		}
	}
		...
		if (wlu_set(irh, WLC_SET_WSEC_PMK, &psk, sizeof(psk))) {
			printf("wlmSecuritySet - error setting SET_WSEC_PMK ioctl: %s\n", wlmLastError());
			return FALSE;
		}
	}
	...
	if (wlu_set(irh, WLC_SET_WSEC, &wsec, sizeof(wsec)) < 0) {
		printf("wlmSecuritySet - error setting SET_WSEC ioctl: %s\n", wlmLastError());
		return FALSE;
	}
...
int wlmIovarSet(const char *iovar, void *buf, int len)
{
	if (wlu_iovar_set(irh, iovar, buf, len)) {
		printf("wlmIovarSet - %s (len:%d): %s\n", iovar, len, wlmLastError());
		return FALSE;
	}
	return TRUE;
}
...
int wlmDhdInit(const char *chip)
{
	...
	printf("firmware download init complete\n");
	    ret = TRUE;
